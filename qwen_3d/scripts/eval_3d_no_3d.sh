...
lora_type=merger_save_all_lora-new_label-after-multi_points-text_guide-2
norm_type=multi_points
grid_n=7
...
while getopts "t:l:n:m:w:d:f:e:" opt; do
  case $opt in
    t) test_json="$OPTARG" ;;
    l) lora_type="$OPTARG" ;;
    n) norm_type="$OPTARG" ;;
...
CUDA_VISIBLE_DEVICES=3 python qwen_3d/eval/eval_3d.py \
    --mode batch \
