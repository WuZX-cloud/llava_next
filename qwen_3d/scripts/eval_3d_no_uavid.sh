...
lora_type=merger_save_all_lora-new_label-after-no-uavid
norm_type=none
grid_n=7
...
test_json=processed_data_with_depth_npy/test/test_data_multi_view_follow_no_uavid.json
...
output_dir=output_3d/${lora_type}/test_results-no-sampled-${output_d}
# 获取当前时间戳
timestamp=$(date +"%Y%m%d_%H%M%S")
output_name=${lora_type}-no-sampled-test_result-${timestamp}
