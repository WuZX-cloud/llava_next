...
grid_n=7
merge_type=direct_add   # direct_add vision_guide text_guide deep_text_guide concat_vision concat_text
is_w2c=True
type_3d=nf  # sincos or nf
enable_3d=True

train_data=./processed_data_with_depth_npy/train/train_data_multi_view_cot.json
...
### 新增加载老权重的参数 
old_checkpoint_dir="output_3d/merger_save_all_lora-gradient_1_gpu_4-v4-3.30-cot-text_cross_attention"


export PYTHONPATH=$(pwd)

CUDA_VISIBLE_DEVICES="${cuda_devices}" deepspeed --master_port=${port} qwen_3d/train/train_3d_from_checkpoint.py  \
      --model_name models/Qwen2.5-VL-7B-Instruct \
      --train_data ${train_data} \
      --video_mapping ${video_mapping} \
      --vision_output_layer visual.merger \
      --system_prompt "$SYSTEM_PROMPT" \
      --pretrained_lora_path ${old_checkpoint_dir} \
