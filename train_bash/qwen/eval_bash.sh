...
# 日志目录
log_dir="./train_bash/qwen/eval_logs"
mkdir -p ${log_dir}
...
# 创建总日志文件
summary_log="${log_dir}/eval_summary_${timestamp}.log"
...
for lora_type in "${lora_types[@]}"; do
    ...
    # 定义每个训练的详细日志文件
    detail_log="${log_dir}/eval_${lora_type}_${timestamp}.log"
    
    # 运行训练脚本并保存日志
    bash qwen_vl_finetune/scripts/eval_batch-lora.sh -l ${lora_type} 2>&1 | tee ${detail_log}
